...
public final class CollectorTest {
  ...
  public void test() {
    ...
    Mockito.when(mockedMemoryMxBean.getNonHeapMemoryUsage())
        .thenReturn(new MemoryUsage(4, 5, 6, 7));
    ...
    {
      GarbageCollectorMXBean mockedGarbageCollectorMxBean =
          Mockito.mock(GarbageCollectorMXBean.class);
    ...
    try (MockedStatic<ManagementFactory> mockedStatic =
        Mockito.mockStatic(ManagementFactory.class)) {
      mockedStatic.when(ManagementFactory::getMemoryMXBean).thenReturn(mockedMemoryMxBean);
      ...
      assertThat(
          metricsNg.getGcMetrics().get(HerokuJvmMetrics.Gc.G1OldGeneration),
          is(equalTo(new HerokuJvmMetrics.GcMetrics(10, 0.1))));
      assertThat(
          metricsNg.getGcMetrics().get(HerokuJvmMetrics.Gc.G1YoungGeneration),
          is(equalTo(new HerokuJvmMetrics.GcMetrics(20, 0.2))));
