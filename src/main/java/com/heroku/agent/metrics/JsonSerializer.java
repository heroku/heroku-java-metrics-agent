...
public final class JsonSerializer {
    /**
     * Serializes Heroku JVM metrics into their JSON representation required by the Heroku service for dyno language
     * metrics.
     *
     * @param metrics The metrics to serialize.
     * @param previousMetrics The metrics before the current metrics. Used to calculate deltas, can be null.
    ...
    public static String serialize(HerokuJvmMetrics metrics, HerokuJvmMetrics previousMetrics) {
        ...
        //
        // In general, this would be a bad idea and shouldn't be attempted. In the case of the metrics agent however, we
        // have a strong case for not using a JSON library here. First, we don't have to write our own escaping logic:
        //
        ...
        //
        // - These libraries add a non-trivial amount of code to the customer's Java process (Jackson: ~2MiB), affecting
        //   available memory. Considering that the smallest dyno only provides 512MB of RAM, adding 2MB is significant.
        // - Since these libraries handle much more than dumb writing of JSON, the attack surface is larger, and we'd
        //   have to deal with security issues from time to time in these libraries.
        // - They would pollute the classpath, requiring us to add additional build configuration that "shades" these
        ...
        if (previousMetrics != null) {
            stringBuilder.append(Math.max(0, previousMetrics.getTotalGcCollectionCount() - metrics.getTotalGcCollectionCount()));
        } else {
            stringBuilder.append(metrics.getTotalGcCollectionCount());
        }

        for (Map.Entry<HerokuJvmMetrics.Gc, HerokuJvmMetrics.GcMetrics> gcMetrics : metrics.getGcMetrics().entrySet()) {
            ...
            if (previousMetrics == null) {
                ...
                HerokuJvmMetrics.GcMetrics previousGcMetrics = previousMetrics.getGcMetrics().get(gcMetrics.getKey());
                if (previousGcMetrics != null) {
                    reportedCollectionCount = Math.max(0, previousGcMetrics.getCollectionCount() - gcMetrics.getValue().getCollectionCount());
                    reportedCollectionTimeInSeconds = Math.max(0.0, previousGcMetrics.getCollectionTimeInSeconds() - gcMetrics.getValue().getCollectionTimeInSeconds());
                }
            }

            stringBuilder.append(",");
            stringBuilder.append("\"jvm_gc_collection_seconds_count.gc_").append(stringGcName).append("\":");
            stringBuilder.append(reportedCollectionCount);

            stringBuilder.append(",");
            stringBuilder.append("\"jvm_gc_collection_seconds_sum.gc_").append(stringGcName).append("\":");
